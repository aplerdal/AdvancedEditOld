...
namespace MkscEdit.Extract
{
    public class TrackOffset
    {
        ...
        public int Objects;
        public int Overlay;
        public int ItemBoxes;
        public int Endline;
        public int[] TileBlocks;
        public int[] LayoutBlocks;
        public TrackOffset(Track trackid, int address, int tilesetPointerTable, int palette, int tileBehaviours, int objects, int overlay, int itemBoxes, int endline)
        {
            Trackid = trackid;
            Address = address;
            TilesetPointerTable = address+tilesetPointerTable;
            Palette = address + palette;
            TileBehaviours = address + tileBehaviours;
            Objects = address + objects;
            Overlay = address + overlay;
            ItemBoxes = address + itemBoxes;
            Endline = address + endline;

            int PointerTableAbosolute = address + tilesetPointerTable;
            TileBlocks = [
                PointerTableAbosolute+(ushort)(Program.file[(PointerTableAbosolute+1)]<<8|Program.file[PointerTableAbosolute  ]),
                PointerTableAbosolute+(ushort)(Program.file[(PointerTableAbosolute+3)]<<8|Program.file[PointerTableAbosolute+2]),
                PointerTableAbosolute+(ushort)(Program.file[(PointerTableAbosolute+5)]<<8|Program.file[PointerTableAbosolute+4]),
                PointerTableAbosolute+(ushort)(Program.file[(PointerTableAbosolute+7)]<<8|Program.file[PointerTableAbosolute+6]),
            ];
            PointerTableAbosolute = address + 0x100;
            LayoutBlocks = [
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 1 )] << 8 | Program.file[PointerTableAbosolute     ]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 3 )] << 8 | Program.file[PointerTableAbosolute + 2 ]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 5 )] << 8 | Program.file[PointerTableAbosolute + 4 ]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 7 )] << 8 | Program.file[PointerTableAbosolute + 6 ]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 9 )] << 8 | Program.file[PointerTableAbosolute + 8 ]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 11)] << 8 | Program.file[PointerTableAbosolute + 10]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 13)] << 8 | Program.file[PointerTableAbosolute + 12]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 15)] << 8 | Program.file[PointerTableAbosolute + 14]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 17)] << 8 | Program.file[PointerTableAbosolute + 16]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 19)] << 8 | Program.file[PointerTableAbosolute + 18]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 21)] << 8 | Program.file[PointerTableAbosolute + 20]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 23)] << 8 | Program.file[PointerTableAbosolute + 22]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 25)] << 8 | Program.file[PointerTableAbosolute + 24]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 27)] << 8 | Program.file[PointerTableAbosolute + 26]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 29)] << 8 | Program.file[PointerTableAbosolute + 28]),
                PointerTableAbosolute + (ushort)(Program.file[(PointerTableAbosolute + 31)] << 8 | Program.file[PointerTableAbosolute + 30]),
            ];
    ...
    // TODO : Refactor this into an array of Offset Objects rather than a single array
       ...
       /*void RegTrack(int[][] _trackOffsets, byte[] rom, Track trackid, int address, int tilesetPointer, int palette, int tileBehaviours, int objects, int overlay, int itemBoxes, int endline)
        {
            ...
            track[(int)TrackOffsets.Address] = address;
            track[(int)TrackOffsets.TilesetPointerTable] = tilesetPointer;
            
            track[(int)TrackOffset.TrackLayoutPointer] = rom[address + 0x100 + 1] << 8 | rom[address + 0x100];
        ...
        public TrackOffset[] trackoffsets;
        public Offsets(byte[] rom)
        {
            trackoffsets = new TrackOffset[Enum.GetValues(typeof(Track)).Length];
            // commented tracks need to be fixed
            
            trackoffsets = [
                //|                        |        Track           | addr   |tilesetptr
                new TrackOffset(Track.SNESMarioCircuit1, 0x2580d4, 0x00000de4, 0x000029b0, 0x00002a30, 0x00002f94, 0x00002f00, 0x00003064, 0x00003078),
                new TrackOffset(Track.SNESDonutPlains1,  0x25b17c, 0x00001470, 0x00002a34, 0x00002ab4, 0x000030f0, 0x00003000, 0x000031c0, 0x000031d4),
                new TrackOffset(Track.SNESGhostValley1,  0x25e380, 0x00000c38, 0x000018d8, 0x00001958, 0x00001ebc, 0x00001e00, 0x00001f8c, 0x00001fa4),
                new TrackOffset(Track.SNESBowserCastle1, 0x260354, 0x00000e38, 0x00002140, 0x000021c0, 0x000027b4, 0x00002700, 0x00002884, 0x00002898),
                ...
                new TrackOffset(Track.SNESChocoIsland1, 0x264348, 0x00000ff8, 0x000024a4, 0x00002524, 0x0000298c, 0x00002900, 0x00002a5c, 0x00002a70),
                ...
                new TrackOffset(Track.SNESKoopaBeach1, 0x26d33c, 0x00001328, 0x00002410, 0x00002490, 0x00002940, 0x00002900, 0x00002a10, 0x00002a24),
                ...
                new TrackOffset(Track.SNESVanillaLake1, 0x271634, 0x000011a0, 0x0000205c, 0x000020dc, 0x00002520, 0x00002500, 0x000025f0, 0x00002604),
                ...
                new TrackOffset(Track.PeachCircuit, 0x283d04, 0x000036c8, 0x000052a0, 0x00005320, 0x000061b8, 0x00006200, 0x000062d0, 0x000062ec),
                new TrackOffset(Track.ShyGuyBeach, 0x28a020, 0x00003324, 0x00004c54, 0x00004cd4, 0x000062c4, 0x00006300, 0x000063dc, 0x000063fc),
                new TrackOffset(Track.SunsetWilds, 0x29044c, 0x00006a10, 0x00008448, 0x000084c8, 0x0000a51c, 0x0000a500, 0x0000a634, 0x0000a660),
                new TrackOffset(Track.BowserCastle1, 0x29aadc, 0x000025e0, 0x000038c4, 0x00003944, 0x00005058, 0x00005000, 0x0000514c, 0x00005168),
                new TrackOffset(Track.LuigiCircuit, 0x29fc74, 0x00003e10, 0x00005c40, 0x00005cc0, 0x00006dfc, 0x00006e00, 0x00006f24, 0x00006f44),
                new TrackOffset(Track.RiversidePark, 0x2a6be8, 0x00004804, 0x0000639c, 0x0000641c, 0x00007720, 0x00007700, 0x00007848, 0x00007870),
                new TrackOffset(Track.YoshiDesert, 0x2ae488, 0x00003e04, 0x000058c0, 0x00005940, 0x000072c0, 0x00007300, 0x00007408, 0x00007430),
                ...
                new TrackOffset(Track.MarioCircuit, 0x2b8fa8, 0x00002f88, 0x00004ee0, 0x00004f60, 0x00006058, 0x00006000, 0x00006174, 0x00006194),
                ...
                new TrackOffset(Track.RibbonRoad, 0x2c4e3c, 0x00003528, 0x0000459c, 0x0000461c, 0x00006434, 0x00006400, 0x00006564, 0x00006588),
                ...
                new TrackOffset(Track.SnowLand, 0x2cecfc, 0x00003a08, 0x00004fbc, 0x0000503c, 0x000070c8, 0x00007100, 0x000071ec, 0x00007214),
                new TrackOffset(Track.BooLake, 0x2d5f40, 0x00002e20, 0x0000487c, 0x000048fc, 0x00005a04, 0x00005a00, 0x00005b1c, 0x00005b40),
                new TrackOffset(Track.CheeseLand, 0x2dbab0, 0x00003d14, 0x00005574, 0x000055f4, 0x00006d10, 0x00006d00, 0x00006e3c, 0x00006e60),
                new TrackOffset(Track.RainbowRoad, 0x2e2940, 0x00002cac, 0x00003bfc, 0x00003c7c, 0x0000526c, 0x00005200, 0x0000536c, 0x00005398),
                new TrackOffset(Track.SkyGarden, 0x2e7d08, 0x00003944, 0x0000568c, 0x0000570c, 0x000069b8, 0x00006a00, 0x00006ae0, 0x00006b00),
        ...
        public TrackOffset this[Track track]
        {
            get => trackoffsets[(int)track];
            set => trackoffsets[(int)track] = value;
        }
        public TrackOffset this[int track]
        {
            get => trackoffsets[track];
            set => trackoffsets[track] = value;
        }
    }
}